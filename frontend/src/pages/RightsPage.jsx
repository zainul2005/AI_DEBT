...
export default function RightsPage() {
  ...
  useEffect(() => {
    const loadRights = async () => {
      const response = await axios.get('http://localhost:8000/api/rights', { params: { search, category } });
    ...
    <section className="container dashboard-shell">
      ...
      <div className="panel-card wide-card animated-card">
        <div className="search-row">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search rights or guidance" />
        ...
        <div className="stats-grid" style={{ marginTop: '1rem' }}>
          {cards.map((item) => (
            <div key={item.id} className="panel-card">
              ...
              <h3>{item.title}</h3>
              <p className="hero-copy" style={{ marginTop: '0.6rem' }}>{item.summary}</p>
            </div>
          ))}
        </div>
      </div>
