...
export default function FinancialHealthPage() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/financial-health').then((response) => setAnalysis(response.data));
  }, []);
    ...
    <section className="container dashboard-shell">
      ...
      <div className="panel-card wide-card animated-card">
        ...
        <div className="meter-block">
          ...
          <div className="meter"><div style={{ width: `${Math.max(0, 100 - debtRatio)}%` }} /></div>
        </div>
        <div className="meter-block">
          <label>Recovery Momentum</label>
          <div className="meter"><div style={{ width: `${Math.min(100, Math.max(0, score + 10))}%` }} /></div>
        </div>
      </div>
      <div className="panel-card wide-card animated-card">
        <div className="panel-header">
